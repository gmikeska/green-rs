    ...
    match client.get_unspent_outputs(params).await {
        Ok(utxos_by_asset) => {
            for (asset_id, utxos) in &utxos_by_asset {
                ...
                let avg = if utxos.is_empty() { 0 } else { total / utxos.len() as u64 };
                ...
                if !conf_distribution.is_empty() {
                    ...
                    confs.sort();
                    for conf in confs {
                        println!("  {} confirmations: {} UTXOs", conf, conf_distribution[conf]);
                    }
                }
            }
        }
    ...
    match client.get_unspent_outputs(params).await {
        Ok(utxos_by_asset) => {
            if utxos_by_asset.is_empty() {
                ...
                for (asset_id, utxos) in &utxos_by_asset {
                    if asset_id != "btc" {  // Skip Bitcoin, show only Liquid assets
